...
kan1 = Lux.Chain(
    KDense_mult( 4,  4, 2, 4; use_base_act = true, basis_func, normalizer),
    #in order: 4 input nodes, 4 grid, 2 mult nodes, 4 output nodes.
)
...
function plot_save(train_losses_1, train_losses_2, train_losses_3, train_losses_4, test_losses_1, test_losses_2, test_losses_3, test_losses_4, epoch)
    ...
    plt=Plots.plot(train_losses_1, yaxis=:log, label="Outputs 1 and 2 (left)", linestyle=line_style, size=(300, 210), dpi=600, c=:grey0)
    plot!(train_losses_2, yaxis=:log, linestyle=line_style, c=:grey0, label=false)
    plot!(train_losses_3, yaxis=:log, label=false, c=:forestgreen)
    plot!(train_losses_4, yaxis=:log, c=:forestgreen, label=false)
    plot!(test_losses_1, yaxis=:log, label=false, c=:grey0)
    plot!(test_losses_2, yaxis=:log, c=:grey0, label=false)
    plot!(test_losses_3, yaxis=:log, label="Outputs 3 and 4 (right)", linestyle=line_style, c=:forestgreen)
    plot!(test_losses_4, yaxis=:log, linestyle=line_style, c=:forestgreen, label=false)
    xlabel!("Epoch")
    ylabel!("Loss")
    png(plt, string(figpath, "/loss_MultKAN.png"))
end
=#

function plot_save(train_losses_1, train_losses_2, train_losses_3, train_losses_4, test_losses_1, test_losses_2, test_losses_3, test_losses_4, epoch)

    plt=Plots.plot(train_losses_1, yaxis=:log, c=:grey30, label="\$z_1\$ train", legend=false, dpi=600, size=(250, 140), linestyle=line_style)
    plot!(train_losses_2, yaxis=:log, c=:snow4, label="\$z_2\$ train")
    plot!(train_losses_3, yaxis=:log, c=:forestgreen, label="\$z_3\$ train", linestyle=line_style)
    plot!(train_losses_4, yaxis=:log, c=:mediumseagreen, label="\$z_4\$ train", ylims=[1e-5, 1], framestyle = :box, grid=false)
    xlabel!("Epoch")
    ylabel!("Loss")
    png(plt, string(figpath, "/loss_MultKAN.png"))
end
...
x3=collect(0:0.01:1)
x4=collect(0:0.01:1)
acts_x1=zeros(length(x1), 6)
acts_x2=zeros(length(x2), 6)
acts_x3=zeros(length(x3), 6)
acts_x4=zeros(length(x4), 6)
...
for i in 1:6 #for each of 4 nodes, there is an X1, X2, X3, X4 plot..
    ...
    png(plt, string(dir, "activation_plots/multkan_X2", string(i), ".png"))
    ...
    png(plt, string(dir, "activation_plots/multkan_X3", string(i), ".png"))
    ...
    png(plt, string(dir, "activation_plots/multkan_X4", string(i), ".png"))
end